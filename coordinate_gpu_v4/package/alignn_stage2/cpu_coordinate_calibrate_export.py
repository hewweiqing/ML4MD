...
from .cpu_prediction_schema import probability as stable_positive_probability, write_csv
from .cpu_coordinate_training import build_or_load_graphs, evaluate_raw, model_config, validate_frozen_files
...
def rows_for(ids, labels, raw_logits, scaled_logits, fold, seed, condition, split, checkpoint_hash,
    ...
    return [{"structure_id": structure_id, "fold": fold, "seed": seed, "condition": condition, "split": split,
        ...
        "sample_order_index": index, "execution_device": "cpu"} for index, structure_id in enumerate(ids)]
...
def main(argv=None) -> int:
    ...
    validation_rows = []
    validation_rows += rows_for(control["structure_ids"], control["labels"], control["logits"], control["logits"], args.fold, args.seed, "CPU_CONTROL_RAW", "inner_validation", control_hash, "raw:T=1", package_hash, coordinate_config_hash)
    validation_rows += rows_for(control["structure_ids"], control["labels"], control["logits"], scaled_control_validation, args.fold, args.seed, "CPU_CONTROL_TS", "inner_validation", control_hash, control_ref, package_hash, coordinate_config_hash)
    validation_rows += rows_for(random2["structure_ids"], random2["labels"], random2["logits"], random2["logits"], args.fold, args.seed, "CPU_RANDOM2_COORDINATE_RAW", "inner_validation", random2_hash, "raw:T=1", package_hash, coordinate_config_hash)
    validation_rows += rows_for(random2["structure_ids"], random2["labels"], random2["logits"], scaled_random2_validation, args.fold, args.seed, "CPU_RANDOM2_COORDINATE_TS", "inner_validation", random2_hash, random2_ref, package_hash, coordinate_config_hash)
    test_rows = []
    test_rows += rows_for(test_ids, test_labels, control_test, control_test, args.fold, args.seed, "CPU_CONTROL_RAW", "outer_test", control_hash, "raw:T=1", package_hash, coordinate_config_hash)
    test_rows += rows_for(test_ids, test_labels, control_test, scaled_control_test, args.fold, args.seed, "CPU_CONTROL_TS", "outer_test", control_hash, control_ref, package_hash, coordinate_config_hash)
    test_rows += rows_for(test_ids, test_labels, random2_test, random2_test, args.fold, args.seed, "CPU_RANDOM2_COORDINATE_RAW", "outer_test", random2_hash, "raw:T=1", package_hash, coordinate_config_hash)
    test_rows += rows_for(test_ids, test_labels, random2_test, scaled_random2_test, args.fold, args.seed, "CPU_RANDOM2_COORDINATE_TS", "outer_test", random2_hash, random2_ref, package_hash, coordinate_config_hash)
    ...
    write_json(work / "COMPLETE.json", {"status": "complete", "fold": args.fold, "seed": args.seed,
        "execution_device": "cpu", "trained_models": ["control", "random2_coordinate"],
        "reported_conditions": ["CPU_CONTROL_RAW", "CPU_CONTROL_TS", "CPU_RANDOM2_COORDINATE_RAW",
            "CPU_RANDOM2_COORDINATE_TS"], "coordinate_sigma_authorization_sha256": authorization.authorization_sha256,
