...
from .cpu_prediction_schema import read_csv
...
CONDITIONS = (
    "CPU_CONTROL_RAW",
    "CPU_CONTROL_TS",
    "CPU_RANDOM2_COORDINATE_RAW",
    "CPU_RANDOM2_COORDINATE_TS",
)
...
def main(argv=None) -> int:
    ...
    result = {"status": "complete", "execution_device": "cpu", "folds": list(FOLDS), "seeds": list(SEEDS),
    ...
    write_json(output_dir / "CPU_COORDINATE_OOF_ANALYSIS.json", result)
    with (output_dir / "cpu_coordinate_oof_predictions.csv").open("w", encoding="utf-8", newline="") as stream:
