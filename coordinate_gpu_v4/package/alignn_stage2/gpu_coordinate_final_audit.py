"""Persistent fail-closed audit for the 25-cell GPU coordinate experiment."""
...
def main(argv=None) -> int:
    ...
    for fold in FOLDS:
        for seed in SEEDS:
            ...
            try:
                ...
                if complete.get("execution_device") != "cuda" or complete.get("coordinate_sigma_authorization_sha256") != authorization.authorization_sha256:
    ...
    oof_path = oof_dir / "GPU_COORDINATE_OOF_ANALYSIS.json"
    if not oof_path.is_file():
        failures.append({"cell": "OOF", "error": "GPU_COORDINATE_OOF_ANALYSIS.json missing"})
    else:
    ...
    report = {"status": "passed" if not failures and len(cells) == 25 else "failed",
        "execution_device": "cuda", "valid_cells": len(cells), "expected_cells": 25,
    ...
    write_json(oof_dir / "GPU_COORDINATE_FINAL_AUDIT.json", report)
    if report["status"] != "passed":
        raise RuntimeError(f"GPU coordinate final audit failed: {failures}")
    print("ALIGNN_GPU_COORDINATE_FINAL_AUDIT: PASS")
    return 0
