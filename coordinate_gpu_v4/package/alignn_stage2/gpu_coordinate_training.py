...
def capture_rng_state() -> dict:
    return {"python_random_state": random.getstate(), "numpy_random_state": np.random.get_state(),
        "torch_cpu_rng_state": torch.get_rng_state(), "torch_cuda_rng_state_all": torch.cuda.get_rng_state_all()}


def restore_rng_state(state: dict) -> None:
    required = {"python_random_state", "numpy_random_state", "torch_cpu_rng_state", "torch_cuda_rng_state_all"}
    if set(state) != required:
        raise RuntimeError("resume checkpoint does not contain the complete Python/NumPy/CPU/CUDA RNG state")
...
def graph_batch(atom_graphs, line_graphs, indices):
    return dgl.batch([atom_graphs[i] for i in indices]).to(DEVICE), dgl.batch([line_graphs[i] for i in indices]).to(DEVICE), None
...
def evaluate_raw(model, atom_graphs, line_graphs, labels):
    ...
    with torch.no_grad():
        for indices in iter_batches(list(range(len(labels)))):
            batch_labels = torch.tensor([labels[index] for index in indices], device=DEVICE)
...
def create_start_states(train_atoms, train_lines, seed: int, work_dir: Path):
    ...
    if base_path.exists():
        base_payload = torch.load(base_path, map_location=DEVICE)
        random2_payload = torch.load(random2_path, map_location=DEVICE)
    ...
    seed_all(seed)
    base = ALIGNN(model_config()).to(DEVICE)
    ...
    descriptors = torch.cat(descriptors).to(DEVICE)
    ...
    def warm_once():
        random2 = ALIGNN(model_config()).to(DEVICE)
        ...
        holdout_generator = torch.Generator(device=DEVICE).manual_seed(seed + 10001)
        ...
        synthetic = mean + std * torch.randn((synthetic_count, descriptors.shape[1]), generator=generator, device=DEVICE)
        labels = torch.tensor([0, 1] * (synthetic_count // 2), device=DEVICE)
        permutation = torch.randperm(synthetic_count, generator=generator, device=DEVICE)
        ...
        for _ in range(warmup_steps):
            ...
            optimizer.step()
        heldout = mean + std * torch.randn((synthetic_count, descriptors.shape[1]), generator=holdout_generator, device=DEVICE)
        with torch.no_grad():
...
def create_coordinate_start_states(fold: int, seed: int, work_dir: Path, coordinate_cache_root: Path,
        authorization):
    base_path, random2_path = work_dir / "paired_gpu_base.pt", work_dir / "coordinate_gpu_warmup_only.pt"
    if base_path.exists() != random2_path.exists():
        raise RuntimeError("incomplete paired GPU start-state artifacts")
    if base_path.exists():
        base_payload, random2_payload = torch.load(base_path, map_location=DEVICE), torch.load(random2_path, map_location=DEVICE)
        ...
        if not states_byte_identical(component_state(base_payload["model"], classifier=False),
            ...
            raise RuntimeError("resumed GPU paired encoders differ")
        return base_payload["model"], random2_payload["model"]
    seed_all(seed)
    base = ALIGNN(model_config()).to(DEVICE); assert_model_gpu(base)
    ...
    random2 = ALIGNN(model_config()).to(DEVICE); random2.load_state_dict(initialized); assert_model_gpu(random2)
    ...
    labels = torch.tensor([int(row["random_label"]) for row in records], dtype=torch.long, device=DEVICE)
    # Preserve the frozen CPU implementation's exact warm-up sample order.
    # Only the selected batch indices cross to CUDA; the permutation RNG does not.
    ...
    for step in range(938):
        ...
        cpu_indices, cursor = permutation[cursor:cursor + 128], cursor + 128
        indices = cpu_indices.to(device=DEVICE, non_blocking=False)
    ...
    changed = {name for name in initialized if not torch.equal(
        initialized[name].detach().cpu(), random2.state_dict()[name].detach().cpu())}
    ...
    write_json(work_dir / "coordinate_warmup_diagnostics.json", {"status": "passed", "fold": fold, "seed": seed,
        ...
        "history": history, "true_labels_used": False, "execution_device": "cuda",
...
def train_branch_gpu(branch: str, start_state, train_atoms, train_lines, train_labels, train_ids,
    ...
    seed_all(seed); model = ALIGNN(model_config()).to(DEVICE); model.load_state_dict(start_state); assert_model_gpu(model)
    ...
    while epoch < EPOCHS:
        ...
        for position in range(batch_position, len(batches)):
            ...
            labels = torch.tensor([train_labels[index] for index in indices], dtype=torch.long, device=DEVICE)
            ...
            optimizer_steps += 1; epoch_loss_sum += float(loss.detach().cpu()) * len(indices); epoch_sample_count += len(indices)
    ...
    best = torch.load(best_path, map_location=DEVICE); model.load_state_dict(best["model"])
    logits = evaluate_raw(model, validation_atoms, validation_lines, validation_labels)
    reconstructed = ALIGNN(model_config()).to(DEVICE); reconstructed.load_state_dict(
        torch.load(best_path, map_location=DEVICE)["model"]); assert_model_gpu(reconstructed)
    ...
    if reconstruction_max_abs_diff > 1e-7:
        raise RuntimeError("selected GPU checkpoint reconstruction gate failed")
    ...
    write_json(branch_dir / "checkpoint_provenance.json", {"branch": branch, "selected_epoch": best["epoch"],
        ...
        "elapsed_runtime_seconds": elapsed_runtime_seconds + time.monotonic() - invocation_started,
        "execution_device": "cuda", "resume_contract": "batch-position exact on same A100 CUDA runtime", **integrity})
    return history, True
...
def train_branch(branch: str, start_state, train_atoms, train_lines, train_labels, train_ids,
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for indices in iter_batches(order):
            optimizer.zero_grad(set_to_none=True)
            labels = torch.tensor([train_labels[index] for index in indices], device=DEVICE)
            log_probabilities, _ = forward_raw(model, graph_batch(train_atoms, train_lines, indices), labels)
    ...
    logits = evaluate_raw(model, validation_atoms, validation_lines, validation_labels)
    reconstructed = ALIGNN(model_config()).to(DEVICE)
    reconstructed.load_state_dict(torch.load(best_path, map_location=DEVICE)["model"])
    reconstructed_logits = evaluate_raw(reconstructed, validation_atoms, validation_lines, validation_labels)
    reconstruction_max_abs_diff = float(np.max(np.abs(logits - reconstructed_logits)))
    if reconstruction_max_abs_diff > 1e-7:
    ...
    write_json(branch_dir / "checkpoint_provenance.json", {"branch": branch, "selected_epoch": best["epoch"],
        ...
        "resume_state": ["Python RNG", "NumPy RNG", "torch CPU RNG", "torch CUDA RNG", "sample-order generator"]})
    return history
...
def main(argv=None) -> int:
    ...
    if (work_dir / "COMPLETE.json").exists():
        ...
        if (complete.get("status"), complete.get("fold"), complete.get("seed"), complete.get("execution_device")) \
                == ("complete", args.fold, args.seed, "cuda"):
            print(f"Verified GPU COMPLETE.json exists; skipping fold={args.fold} seed={args.seed}")
            return 0
    ...
    if args.condition in ("control", "paired"):
        ...
        histories["control"], complete = train_branch_gpu("control", base_state, train_atoms, train_lines, train_labels, train_ids,
    ...
    if args.condition in ("random2", "paired"):
        if "integrity" not in locals():
            integrity = {"package_aggregate_sha256": json.loads((PACKAGE_ROOT / "PACKAGE_MANIFEST.json").read_text())["aggregate_sha256"],
                ...
                "clean_cache_manifest_sha256": sha256_file(cache_root / "STRUCTURE_CACHE_MANIFEST.json"),
                "coordinate_cache_manifest_sha256": sha256_file(coordinate_cache_root / f"fold_{args.fold}" / f"seed_{args.seed}" / "COORDINATE_CACHE_MANIFEST.json")}
        histories["random2"], complete = train_branch_gpu("random2_coordinate", random2_state, train_atoms, train_lines, train_labels, train_ids,
            validation_atoms, validation_lines, validation_labels, validation_ids, args.seed, work_dir, integrity)
        if not complete: return 75
