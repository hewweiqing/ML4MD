...
CONDITIONS = {"GPU_CONTROL_RAW", "GPU_CONTROL_TS", "GPU_RANDOM2_COORDINATE_RAW", "GPU_RANDOM2_COORDINATE_TS"}
...
def validate(rows, expected_split=None):
    ...
    for row in rows:
        ...
        if row["execution_device"] != "cuda": raise ValueError("GPU prediction artifact has non-CUDA device")
