...
ROC_AUC_SCORE_INPUT = "continuous positive-class probability p_1; never predicted labels"
...
def metrics(labels: np.ndarray, logits: np.ndarray) -> dict:
    ...
    probabilities = np.where(margin >= 0, 1.0 / (1.0 + np.exp(-margin)), np.exp(margin) / (1.0 + np.exp(margin)))
    ...
    auc = roc_auc_continuous(labels, probabilities)
...
def assert_temperature_invariance(raw: np.ndarray, scaled: np.ndarray, labels: np.ndarray | None = None,
    ...
    checks = {"predictions_unchanged": bool(np.array_equal(np.argmax(raw, axis=1), np.argmax(scaled, axis=1))),
        ...
        "ranking_unchanged": bool(np.array_equal(np.argsort(raw_margin, kind="stable"), np.argsort(scaled_margin, kind="stable"))),
        "raw_finite": bool(np.isfinite(raw).all()), "scaled_finite": bool(np.isfinite(scaled).all()),
        "tie_pattern_unchanged": bool(np.array_equal(np.isclose(raw_margin[:, None], raw_margin[None, :], atol=atol),
                                                     np.isclose(scaled_margin[:, None], scaled_margin[None, :], atol=atol)))}
    if labels is not None:
        raw_probability = np.where(raw_margin >= 0, 1.0 / (1.0 + np.exp(-raw_margin)),
            np.exp(raw_margin) / (1.0 + np.exp(raw_margin)))
        scaled_probability = np.where(scaled_margin >= 0, 1.0 / (1.0 + np.exp(-scaled_margin)),
            np.exp(scaled_margin) / (1.0 + np.exp(scaled_margin)))
        checks["roc_auc_unchanged"] = bool(roc_auc_continuous(labels, raw_probability)
            == roc_auc_continuous(labels, scaled_probability))
