...
from .prediction_schema import stable_positive_probability, write_csv
from .training import build_or_load_graphs, evaluate_raw, model_config, validate_frozen_files
...
def rows_for(ids, labels, raw_logits, fold, seed, condition, split, checkpoint_hash):
    return [{"structure_id": structure_id, "fold": fold, "seed": seed, "condition": condition, "split": split,
        ...
        "raw_probability_positive_audit_only": stable_positive_probability(*raw_logits[index]),
        "predicted_label": int(raw_logits[index, 1] > raw_logits[index, 0]), "checkpoint_sha256": checkpoint_hash,
        "sample_order_index": index} for index, structure_id in enumerate(ids)]
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--numerical-resolution-record")
    ...
    control_path, random2_path = work / "control" / "validation_raw_logits.npz", work / "random2" / "validation_raw_logits.npz"
    ...
    resolution = args.numerical_resolution_record
    fitted_control = fit_temperature(control["logits"], control["labels"], split="validation",
        resolution_record=resolution, resolution_branch="control" if resolution else None,
        validation_artifact_sha256=sha256_file(control_path) if resolution else None)
    fitted_random2 = fit_temperature(random2["logits"], random2["labels"], split="validation",
        resolution_record=resolution, resolution_branch="random2" if resolution else None,
        validation_artifact_sha256=sha256_file(random2_path) if resolution else None)
    ...
    control_checkpoint, random2_checkpoint = work / "control" / "best.pt", work / "random2" / "best.pt"
    ...
    validation_rows = []
    validation_rows += rows_for(control["structure_ids"], control["labels"], control["logits"], args.fold, args.seed, "A_Control_raw", "inner_validation", control_hash)
    validation_rows += rows_for(control["structure_ids"], control["labels"], control["logits"], args.fold, args.seed, "B_Control_temperature_scaled", "inner_validation", control_hash)
    validation_rows += rows_for(random2["structure_ids"], random2["labels"], random2["logits"], args.fold, args.seed, "C_Random2_Descriptor_raw", "inner_validation", random2_hash)
    validation_rows += rows_for(random2["structure_ids"], random2["labels"], random2["logits"], args.fold, args.seed, "D_Random2_Descriptor_temperature_scaled", "inner_validation", random2_hash)
    test_rows = []
    test_rows += rows_for(test_ids, test_labels, control_test, args.fold, args.seed, "A_Control_raw", "outer_test", control_hash)
    test_rows += rows_for(test_ids, test_labels, control_test, args.fold, args.seed, "B_Control_temperature_scaled", "outer_test", control_hash)
    test_rows += rows_for(test_ids, test_labels, random2_test, args.fold, args.seed, "C_Random2_Descriptor_raw", "outer_test", random2_hash)
    test_rows += rows_for(test_ids, test_labels, random2_test, args.fold, args.seed, "D_Random2_Descriptor_temperature_scaled", "outer_test", random2_hash)
