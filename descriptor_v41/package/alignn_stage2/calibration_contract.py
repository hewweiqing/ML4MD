...
class FittedTemperature:
    ...
    numerical_resolution_authorized: bool = False
    numerical_resolution_record_sha256: str | None = None
    ...
    def validate(self) -> "FittedTemperature":
        ...
        if not isinstance(self.converged, (bool, np.bool_)):
            raise TemperatureScalingContractError("temperature fitting convergence must be Boolean")
        if not bool(self.converged) and not self.numerical_resolution_authorized:
            raise TemperatureScalingContractError("temperature fitting must converge or carry an authorized v40 resolution record")
        if self.numerical_resolution_authorized:
            if bool(self.converged) or self.convergence_status != "not_converged":
                raise TemperatureScalingContractError("v40 resolution must preserve the original nonconverged result")
            if not self.numerical_resolution_record_sha256 or len(self.numerical_resolution_record_sha256) != 64:
                raise TemperatureScalingContractError("v40 resolution record provenance is incomplete")
...
def fit_temperature(validation_logits: Any, validation_labels: Any, *, split: str = "validation",
        approval_path: str | Path = DEFAULT_APPROVAL, resolution_record: str | Path | None = None,
        resolution_branch: str | None = None, validation_artifact_sha256: str | None = None) -> FittedTemperature:
    if split != "validation":
    ...
    if isinstance(fitted, dict):
        ...
        result = FittedTemperature(source_sha256=digest, implementation_object=fitted,
            convergence_status=str(fitted.get("convergence_status", "converged" if fitted["converged"] else "not_converged")),
            **{key: fitted[key] for key in required})
    else:
        ...
        result = FittedTemperature(source_sha256=digest, implementation_object=fitted,
            convergence_status=str(getattr(fitted, "convergence_status", "converged" if getattr(fitted, "converged") else "not_converged")),
            **{key: getattr(fitted, key) for key in required})
    if not bool(result.converged):
        if resolution_record is None or resolution_branch is None or validation_artifact_sha256 is None:
            return result.validate()
        if not isinstance(fitted, dict):
            raise TemperatureScalingContractError("v40 resolution requires the approved MUBen dictionary fit object")
        from .numerical_resolution import NumericalResolutionError, validate_resolution_record
        try:
            record_sha256, branch_record = validate_resolution_record(
                resolution_record, branch=resolution_branch, source_sha256=digest,
                validation_artifact_sha256=validation_artifact_sha256, fitted=fitted)
        except NumericalResolutionError as error:
            raise TemperatureScalingContractError(str(error)) from error
        result = FittedTemperature(source_sha256=digest, implementation_object=fitted,
            convergence_status=str(branch_record["original_convergence_status"]),
            numerical_resolution_authorized=True,
            numerical_resolution_record_sha256=record_sha256,
            **{key: fitted[key] for key in required})
    return result.validate()
