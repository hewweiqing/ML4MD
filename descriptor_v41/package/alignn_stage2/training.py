...
from .production import verify_complete
...
V26_CACHE_RELEASE_IDENTITY = "1c15f877cee4eb6eebfd096962c6a793d0d3f14385287a6a9385e3adce05bd7d"
...
def graph_batch(atom_graphs, line_graphs, indices):
    return dgl.batch([atom_graphs[i] for i in indices]).to("cuda"), dgl.batch([line_graphs[i] for i in indices]).to("cuda"), None
...
def evaluate_raw(model, atom_graphs, line_graphs, labels):
    ...
    with torch.no_grad():
        for indices in iter_batches(list(range(len(labels)))):
            batch_labels = torch.tensor([labels[index] for index in indices], device="cuda")
...
def cache_expected_provenance(dataset: Path) -> dict:
    ...
    return expected_provenance(GRAPH_SETTINGS, jarvis_version=JARVIS_VERSION, dgl_version=dgl.__version__,
        release_identity=V26_CACHE_RELEASE_IDENTITY)
...
def create_start_states(train_atoms, train_lines, seed: int, work_dir: Path):
    ...
    seed_all(seed)
    base = ALIGNN(model_config()).to("cuda")
    ...
    def warm_once():
        ...
        holdout_generator = torch.Generator(device="cuda").manual_seed(seed + 10001)
        ...
        labels = torch.tensor([0, 1] * (synthetic_count // 2), device="cuda")
        ...
        for _ in range(warmup_steps):
            ...
            optimizer.step()
        heldout = mean + std * torch.randn((synthetic_count, descriptors.shape[1]), generator=holdout_generator, device="cuda")
        with torch.no_grad():
...
def train_branch(branch: str, start_state, train_atoms, train_lines, train_labels, train_ids,
    ...
    if last_path.is_file():
        checkpoint = torch.load(last_path, map_location="cuda")
    ...
    best = torch.load(best_path, map_location="cuda")
    model.load_state_dict(best["model"])
    logits = evaluate_raw(model, validation_atoms, validation_lines, validation_labels)
    reconstructed = ALIGNN(model_config()).to("cuda")
    reconstructed.load_state_dict(torch.load(best_path, map_location="cuda")["model"])
    ...
    write_json(branch_dir / "checkpoint_provenance.json", {"branch": branch, "selected_epoch": best["epoch"],
        ...
        "resume_state": ["Python RNG", "NumPy RNG", "torch CPU RNG", "all CUDA RNG states", "sample-order generator"]})
    return history
...
def main(argv=None) -> int:
    ...
    if not torch.cuda.is_available() or dgl.__version__ != "1.1.1+cu118":
        raise RuntimeError("frozen CUDA/DGL gate failed")
    ...
    if verify_complete(work_dir, args.fold, args.seed):
        print(f"Verified COMPLETE.json exists; skipping fold={args.fold} seed={args.seed}")
        return 0
    ...
    base_state, random2_state = create_start_states(train_atoms, train_lines, args.seed, work_dir)
    ...
    if args.condition in ("control", "paired"):
        histories["control"] = train_branch("control", base_state, train_atoms, train_lines, train_labels, train_ids,
            validation_atoms, validation_lines, validation_labels, validation_ids, args.seed, work_dir)
    if args.condition in ("random2", "paired"):
        histories["random2"] = train_branch("random2", random2_state, train_atoms, train_lines, train_labels, train_ids,
            validation_atoms, validation_lines, validation_labels, validation_ids, args.seed, work_dir)
