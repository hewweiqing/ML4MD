"""Fail-closed Random2-Coordinate sigma authorization boundary for stage3_calibration.

Self-contained duplicate of coordinate_gpu_v4's sigma_authorization.py, not a
cross-package reference (see GPU_COORDINATE_V29_AUTHORIZATION_BINDING_SOURCE.md
at the package root for why, and for the provenance trail back to the original
coordinate_gpu_v4 authorization/binding artifacts this reuses).
"""
...
BINDING_PATH = PACKAGE_ROOT / "STAGE3_COORDINATE_SIGMA_AUTHORIZATION_BINDING.json"
...
def require_authorized_sigma(*, authorization_path: str | Path | None = None,
    ...
    if test_config is not None:
        if not allow_test_only or os.getenv("STAGE3_COORDINATE_TEST_MODE") != "1":
    ...
    path = Path(authorization_path or os.getenv("STAGE3_COORDINATE_SIGMA_AUTHORIZATION", ""))
    if not str(path) or not path.is_file():
        raise CoordinateSigmaAuthorizationMissing(
            "pending_coordinate_sigma_authorization: set STAGE3_COORDINATE_SIGMA_AUTHORIZATION to a verified prospective authority artifact")
